...
// Custom Tile Layer
// Şehir isimlerini göstermeyen ancak sınırları belirgin olan harita katmanı
...
// List of Turkish cities with coordinates
const cities = [ 
    { name: "Adana", lat: 37.0, lon: 35.3213 },
            ...
            { name: "Tunceli", lat: 39.1056, lon: 39.5481 },
            { name: "Uşak", lat: 38.6823, lon: 29.4082 },
            { name: "Van", lat: 38.5012, lon: 43.402 },
            { name: "Yalova", lat: 40.6558, lon: 29.2769 },
...
// Doğru tahmin edilen şehir için yeşil marker ekle
// Yeşil marker ikonu
function addGreenMarker(city) {
    const greenIcon = L.icon({
        iconUrl: 'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-green.png', // Yeşil marker URL'si
        iconSize: [25, 41],
        iconAnchor: [12, 41],
        popupAnchor: [1, -34],
        shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-shadow.png', // Gölge ikonu
        shadowSize: [41, 41]
    });
    L.marker([city.lat, city.lon], { icon: greenIcon }).addTo(map); // Yeşil marker ekle
}
...
submitBtn.addEventListener("click", function () {
    ...
    if (guess === currentCity) {
        updateScore();
        addGreenMarker(cities[currentCityIndex]); // Doğru bilinen şehre yeşil marker ekle
        currentCityIndex = Math.floor(Math.random() * cities.length);
        placeMarker(cities[currentCityIndex]);
        cityInput.value = "";
